...
export async function autoSplitActiveMachineLogs() {
  ...
  try {
    ...
    const activeLogs = raw.map((l: any) => ({
      id: l._id.toString(),
      ...
      while (maxIterations-- > 0) {
        ...
        if (endOfDay.getTime() < now.getTime()) {
          ...
          await db.collection('MachineLog').updateOne(
            { _id: new mongoose.Types.ObjectId(currentLogId) },
            { $set: { endTime: endOfDay, status: 'completed', remarks, updatedAt: new Date() } }
          );
          
          // Increment machine total run hours
          if (runHours > 0) {
            await db.collection('Machine').updateOne(
              { _id: new mongoose.Types.ObjectId(log.machineId) },
              { $inc: { totalRunHours: runHours }, $set: { updatedAt: new Date() } }
            );
          }
